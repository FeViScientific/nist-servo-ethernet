...
//
// phase_out is a sync flag for the sweep: high during the rising half of the
// triangle (GOINGUP, minval->maxval) and low during the falling half
// (GOINGDOWN, maxval->minval), so its edges land at the sweep turnarounds. It
// is forced low whenever the sweep is disabled (on_in == 0).
//
///////////////////////////////////////////////////////////////////////////////
...
module Sweep #(
   ...
   output reg  signed	[SIGNAL_OUT_SIZE+1:0]	signal_out,
	output reg												phase_out	// 1 = rising half (sync), 0 = falling/off
);
...
always @(posedge clk_in) begin
	...
	phase_out  <= on_in ? (state_f == GOINGUP) : 1'b0;
end
