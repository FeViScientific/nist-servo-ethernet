...
module PhaseDetector(
   input  wire											clk_in,			// 100 MHz for PLL logic
	input  wire											clk_dsp_in,		// 25 MHz for signal processing
...
// Mixers (run on slow clock)
reg signed [31:0] sigXcos, sigXsin;

always @(posedge clk_dsp_in) begin
	sigXcos <= signal_in*cos;
	sigXsin <= signal_in*sin;
end

// Low pass filters (run on slow clock)
LPfilter #(
	.SIGNAL_IN_SIZE(32),
	.SIGNAL_OUT_SIZE(32)
)
...
// arctan runs on slow clock since inputs only update at that rate
...
// Unwrap the phase (run on slow clock)
// Note: rst is synchronous to clk_in, need to synchronize to clk_dsp_in
reg rst_dsp_sync1, rst_dsp_sync2;
always @(posedge clk_dsp_in) begin
	rst_dsp_sync1 <= rst;
	rst_dsp_sync2 <= rst_dsp_sync1;
end

always @(posedge clk_dsp_in) begin
	if (rst_dsp_sync2) begin
		phase_out <= 0;
		phaseOld <= 0;
	end
	else begin
