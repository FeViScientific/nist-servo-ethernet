...
// Sign-extend to 32 bits before shift to prevent truncation when SIGNAL_IN_SIZE > 16
wire signed [31:0] signal_in_scaled;
assign signal_in_scaled = $signed(signal_in) <<< (32 - SIGNAL_IN_SIZE);
...
always @(posedge clk_in) begin
	if (on_in) begin
		b1x0 <= b1_in * signal_in_scaled;
		b0x1 <= b0_in * signal_in_scaled + b1x0;
		if ( (hold_in) ||
